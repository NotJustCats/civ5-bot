...
let eloChart;
if (PLAYERS.length) {{
  const _rankLinePlugin = {{
    id: "rankLinePlugin",
    afterDraw(chart) {{
      if (!document.getElementById("rankToggle")?.checked) return;
      const {{ctx, chartArea: {{left, right}}, scales: {{y}}}} = chart;
      const ranks = [
        {{elo:1600, color:"#f97316", label:"DEITY"}},
        {{elo:1400, color:"#a855f7", label:"EMPEROR"}},
        {{elo:1250, color:"#06b6d4", label:"KING"}},
        {{elo:1100, color:"#3b82f6", label:"PRINCE"}},
        {{elo:1000, color:"#22c55e", label:"CHIEFTAIN"}},
      ];
      ctx.save();
      ranks.forEach(r => {{
        if (r.elo < y.min || r.elo > y.max) return;
        const yPos = y.getPixelForValue(r.elo);
        ctx.beginPath();
        ...
        ctx.textAlign = "right";
        ctx.fillText(r.label, right - 4, yPos - 3);
      }});
      ctx.restore();
    }}
  }};
...
async def handle_restore(request):
    """Temporary endpoint to restore ranked_data.json from a POST body."""
    secret = request.query.get("secret", "")
    if secret != "NotJustCats":
        return web.Response(text="Forbidden", status=403)
    try:
        body = await request.json()
        save_all_data(body)
        return web.Response(text=f"Restored {len(body)} guilds OK")
